...
The controls below the viewport contain four independent switches: **Surface points**, **X center SDF plane**, **Y center SDF plane**, and **Z center SDF plane**. Any combination can be visible at once; points and the Z plane are initially enabled. Below these switches, **Fit scene** sits beside the optional **Coordinate axes** switch, which is initially off. X/Y/Z specify the plane normal, so the X slice is parallel to YZ, for example. Toggling layers or axes preserves the camera; **Fit scene** reframes the active geometry.
